...
If the encounter is associated with an administrative case (Fall / Cas / Caso), the `identifier:caseNumber` slice **SHOULD** be used to report it. However, this profile `CHCoreEncounter` does not represent an administrative case itself, it represents the clinical encounter that may be linked to such a case (see [here](StructureDefinition-ch-administrative-case.html) for details). Any identifier related to the visit (Besuchsnummer / Numero della visita | Numéro de visite), **SHOULD** reported using the `identifier:visitNumber` slice.   
